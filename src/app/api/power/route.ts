...
const metersMap: Record<string, string> = {
  wastewater_pump_building1: "pump_building1",
  wastewater_pump_building2: "pump_building2",
  treatment_pond: "treatment_pond",
};
...
function fillData(result: Record<number, Record<string, number>>, data: PowerRow[]) {
  data.forEach(row => {
    const period = Number(row.period);
    const key = metersMap[row.remark_code] || row.remark_code;
    if (!result[period]) result[period] = {};
    result[period][key] = Number(row.sum_diff_kwh?.toFixed(2) || 0);
  });
  return result;
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    if (type === "yearly") {
      ...
      const dataRaw = await prisma.$queryRaw<PowerRow[]>`
        SELECT
          remark_code,
          EXTRACT(YEAR FROM datetime)::int AS period,
          SUM(diff_kwh) AS sum_diff_kwh
        FROM power_log_10min
