...
export default function Page() {
  ...
  useEffect(() => {
    ...
    socket.addEventListener("message", (event) => {
      ...
      setData({
        m3: `${res.air_cooling_water_building2} m3`,
      });
    });
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 pt-0">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full">
        ...
        <div className="bg-muted/50 aspect-video rounded-xl p-4 overflow-auto flex justify-center items-center  h-[250px] w-full">
          <div className="flex flex-col items-center justify-center h-full">
            <StopAlarmButton location="น้ำแอร์รังผึ้งอาคาร 2" status="Stop Alarm"/>
          </div>
        </div>
      </div>
      ...
      <CubeChart
          ...
          dataKey="air_cooling_water_building2"
          minmaxUrl="/api/threshold?tag=น้ำแอร์รังผึ้งอาคาร 2"
        />
      </div>
    </div>
  );
